...
public class NeyroNet : MonoBehaviour {
    ...
    public NeyroNet(int NumberInputs=720, int NumberHiddenNeurons=100, int NumberOutputs=3,
...
public class GeneticAlgorithm : MonoBehaviour {
    ...
    public GeneticAlgorithm() {
        ...
        for(int i=0; i<10; i++) { this.Persons[i] = new NeyroNet(720, 100, 3, "1/"+((i+1).ToString())); }
    }
...
public class ML : MonoBehaviour {
    ...
    void Update() {
        ...
        int Living = 0;
        for(int i=0; i<Players.Length; i++) {
            if(Genetic.Persons[i].Life)
                if(Players[i].transform.position.y < 15f) {
                    Genetic.Persons[i].LifeTime = CounterTime;
                    Genetic.Persons[i].Life = false;
                    print("Destroy: " + Players[i]);
                    Destroy(Players[i]);
                }
                ...
                float[] DistanceToObjects = new float[360];//ObjectsOnSceneInDict.Keys.ToArray();
                string[] TypeObjectsString = new string[360];//ObjectsOnSceneInDict.Values.ToArray();
